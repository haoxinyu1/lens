  ...
  return (
    <div className="grid gap-3">
      {attempts.map((attempt, index) => (
        <Card key={`${attempt.channel_id}-${index}`} className="py-0 shadow-none">
          <CardContent className="px-4 py-4">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div className="flex min-w-0 flex-wrap items-center gap-2">
              <span className="inline-flex h-7 min-w-7 items-center justify-center rounded-full bg-muted px-2 text-xs font-semibold text-muted-foreground">{index + 1}</span>
              <span className="max-w-[220px] truncate text-sm font-medium text-foreground">{attempt.channel_name}</span>
              {attempt.model_name ? <span className="max-w-[220px] truncate text-xs text-muted-foreground">{attempt.model_name}</span> : null}
          ...
          {attempt.error_message ? <div className="mt-3 rounded-md bg-destructive/10 px-3 py-2 text-xs text-destructive">{attempt.error_message}</div> : null}
          </CardContent>
        </Card>
      ))}
    </div>
...
function RequestCard({
  ...
  const modelName = item.upstream_model_name || item.resolved_group_name || item.requested_group_name || ''
        ...
        <CardContent className="grid grid-cols-[auto_minmax(0,1fr)] gap-x-4 gap-y-3 px-5 py-5 lg:px-6">
          ...
          <div className="min-w-0 self-center">
            <div className="flex flex-wrap items-center gap-2 text-sm leading-none">
              <span className="max-w-[220px] truncate font-semibold text-foreground">{item.requested_group_name || item.resolved_group_name || 'n/a'}</span>
              <ProtocolBadge protocol={item.protocol} />
              <StatusBadge success={item.success} locale={locale} />
              {item.upstream_model_name && item.upstream_model_name !== item.requested_group_name ? <span className="max-w-[220px] truncate text-xs text-muted-foreground">{item.upstream_model_name}</span> : null}
            </div>
          </div>
          ...
          {detailLoading || !detail ? (
            ...
            <div className="grid gap-5">
              <div className="grid gap-4 lg:grid-cols-5">
                <DetailStat label={locale === 'zh-CN' ? '模型组' : 'Group'} value={detail.resolved_group_name || (locale === 'zh-CN' ? '未命中' : 'No match')} />
              ...
              <Card className="py-0">
                <CardHeader className="mb-0 px-4 py-4 lg:px-5">
                  <CardTitle className="text-sm font-semibold text-foreground">{locale === 'zh-CN' ? '尝试链路' : 'Attempts'}</CardTitle>
              ...
              <div className="grid gap-5 xl:grid-cols-2">
                <Card className="overflow-hidden py-0">
                  <CardHeader className="border-b px-4 py-4">
                    <CardTitle className="text-sm font-semibold text-foreground">{locale === 'zh-CN' ? '请求内容' : 'Request'}</CardTitle>
                ...
                <Card className="overflow-hidden py-0">
                  <CardHeader className="border-b px-4 py-4">
                    <CardTitle className="text-sm font-semibold text-foreground">{locale === 'zh-CN' ? '响应内容' : 'Response'}</CardTitle>
