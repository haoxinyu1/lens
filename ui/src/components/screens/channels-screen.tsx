...
function activeBaseUrlValue(
  form: FormState,
  protocol: Pick<FormProtocol, "base_url_id">,
) {
...
function readStoredProtocolFilter(siteId: string, protocolId: string) {
  ...
  return window.localStorage.getItem(
    protocolFilterStorageKey(siteId, protocolId),
  );
}

function writeStoredProtocolFilter(
  siteId: string,
  protocolId: string,
...
function inferProtocolFilterCredential(
  siteId: string,
  protocol: SiteProtocolLike,
) {
  ...
  const modelCredentialId =
    protocol.models.find(
      (model) =>
        model.enabled && availableCredentialIds.has(model.credential_id),
    )?.credential_id ??
    protocol.models.find((model) =>
      availableCredentialIds.has(model.credential_id),
    )?.credential_id;
  if (modelCredentialId) {
    return modelCredentialId;
  }

  const firstEnabledBinding = protocol.bindings.find(
      ...
      <div className="flex flex-col gap-3">
        <div className="grid gap-3 xl:grid-cols-[minmax(0,1fr)_minmax(0,1fr)_minmax(0,1fr)_32px_auto] xl:items-end">
            ...
            <NativeSelect
              ...
              onChange={(event) =>
                onUpdateProtocol(protocolIndex, {
                  protocol: event.target.value as ProtocolKind,
                })
              }
            >
              {protocolOptions.map((option) => (
          ...
          <Field>
            ...
            <NativeSelect
              className={selectClassName()}
              value={resolveBaseUrlId(form.base_urls, protocol.base_url_id)}
              onChange={(event) =>
                onUpdateProtocol(protocolIndex, {
          ...
          <div className="flex h-8 w-8 items-center justify-center xl:self-end">
            <SwitchButton
              ...
              onChange={(checked) =>
                onUpdateProtocol(protocolIndex, { enabled: checked })
              }
            />
          </div>
          <div className="flex flex-wrap items-center justify-end gap-2 xl:col-start-5 xl:row-start-1 xl:self-end">
            <Button
              ...
              onClick={() => onOpenAdvanced(protocolIndex)}
            >
              <Ellipsis size={16} />
            </Button>
            <Button
              ...
              onClick={() => onRemoveProtocol(protocolIndex)}
            >
              <X size={16} />
            </Button>
            <Button
          ...
          <div className="grid gap-3 pt-1">
            ...
            <div className="grid gap-3 xl:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]">
                ...
                <div className="grid min-w-0 gap-2 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-end">
                  ...
                  <Button
                    ...
                    disabled={
                      ...
                      !protocol.manual_model_name.trim()
                    }
                  >
              ...
              <FieldGroup className="gap-2">
                ...
                <div className="grid min-w-0 gap-2 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-end">
                  ...
                  <Button
                    type="button"
                    onClick={() => onFetchModels(protocolIndex)}
                    disabled={
                      fetchingProtocolIndex === protocolIndex ||
                      !form.base_urls.some(
                        (item) => item.enabled && item.url.trim(),
                      ) ||
                      !activeCredentialIds.size
                    }
                  >
                    <RefreshCcw
            ...
            <div className="flex items-center justify-between gap-3">
              ...
              <Button
                ...
                variant="destructive"
                size="sm"
                onClick={() => onUpdateProtocol(protocolIndex, { models: [] })}
                  ...
                  {visibleModels.map(({ model, modelIndex }) => (
                    ...
                    >
                      ...
                      <Button
                        ...
                        onClick={() =>
                          onOpenModelTest(protocolIndex, modelIndex)
                        }
                        disabled={
...
export function ChannelsScreen() {
  ...
  const visibleSites = useMemo<SiteRow[]>(() => {
    ...
    const filtered = siteRows.filter((site) => {
      ...
      if (
        protocolFilter !== "all" &&
        !site.protocols.some((item) => item.protocol === protocolFilter)
      )
        return false;
    ...
    setForm((current) => {
      ...
      return {
        ...
        protocols: current.protocols.map((protocol) => ({
          ...protocol,
          bindings: protocol.bindings.filter(
            (binding) => binding.credential_id !== target.id,
          ),
          models: protocol.models.filter(
            (model) => model.credential_id !== target.id,
          ),
          model_filter_credential_id:
            protocol.model_filter_credential_id === target.id
              ? null
              : protocol.model_filter_credential_id,
        })),
      };
  ...
  async function fetchProtocolModels(protocolIndex: number) {
    const protocol = form.protocols[protocolIndex];
    if (!protocol) return;
    const activeCredentials = form.credentials
      .filter((item) => item.enabled && item.api_key.trim())
      .map((item, index) => ({
        ...item,
        display_name: credentialLabel(item, index, locale),
      }));
    const selectedCredentialId = activeCredentials.some(
      (item) => item.id === protocol.model_filter_credential_id,
    )
      ? protocol.model_filter_credential_id || ""
    ...
    try {
      ...
      const payload: SiteModelFetchPayload = {
        ...
        bindings: selectedCredentialId
          ...
          : form.credentials
              .filter((item) => item.enabled && item.api_key.trim())
              .map((item) => ({ credential_id: item.id, enabled: true })),
      };
                            ...
                            <div className="flex flex-wrap items-center gap-2">
                              ...
                              {site.protocols.map((protocol) => (
                                <Badge
                                  ...
                                  className={cn(
                                    "px-2.5 py-0.5",
                                    protocolBadgeClassName(protocol.protocol),
                                  )}
                                >
            ...
            <form className="grid gap-5" onSubmit={submit}>
              <div className="grid gap-4">
                              ...
                              <FieldGroup className="min-w-0 gap-3 md:contents">
                                ...
                                <div className="flex h-8 w-8 items-center justify-center">
                                  <SwitchButton
                                    ...
                                    onChange={(checked) =>
                                      updateBaseUrl(index, { enabled: checked })
                                    }
                                  />
                                </div>
                                <Button
                ...
                <section className="grid gap-4">
                  <div className="mb-4 flex items-center justify-between gap-3">
                    <div className="text-base font-semibold text-foreground">
                      {locale === "zh-CN" ? "协议列表" : "Protocol configs"}
                    </div>
                    <Button
                      type="button"
                      variant="outline"
        ...
        <AdvancedProtocolDialog
          ...
          protocol={
            advancedProtocolIndex !== null
              ? form.protocols[advancedProtocolIndex]
              : undefined
          }
